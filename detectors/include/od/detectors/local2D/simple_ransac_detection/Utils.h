...
namespace od {
  ...
  namespace l2d {
		...
		void viewImage(cv::Mat image, std::vector<cv::KeyPoint> keypoints = std::vector<cv::KeyPoint>(0));
		...
		void drawArrow(cv::Mat image, cv::Point2i p, cv::Point2i q, cv::Scalar color, int arrowMagnitude = 9, 
