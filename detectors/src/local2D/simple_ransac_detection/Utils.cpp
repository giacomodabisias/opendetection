...
//    skip = 10;
  ...
  for( size_t i = 0; i < list_triangles.size(); i+= skip)
  {
...
void drawObjectMesh1(cv::Mat image, const Mesh *mesh, const Model *model, PnPProblem *pnpProblem, cv::Scalar color)
{
  ...
  cv::projectPoints(model->get_points3d(), pnpProblem->get_R_vect(), pnpProblem->get_t_matrix(), pnpProblem->get_A_matrix(), pnpProblem->get_dist_coef(), imgpoints);
