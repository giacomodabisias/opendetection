#include "od/detectors/global2D/localization/SelectiveSearchModel.h"

namespace od
{
	namespace g2d
	{
		SelectiveSearchModel::SelectiveSearchModel(const std::string & trained_data_location):
		                     					   Detector2D(trained_data_location_){}

		void SelectiveSearchModel::setLabel(int l)
		{
			label = l;
		}

		void SelectiveSearchModel::init()
		{
		}

		shared_ptr<Detections2D> SelectiveSearchModel::detectOmni(shared_ptr<SceneImage> scene)
		{
			return make_shared<Detections2D>();
		}

		shared_ptr<Detections> SelectiveSearchModel::detect(shared_ptr<SceneImage> scene)
		{
			return make_shared<Detections>();		
		}

		shared_ptr<Detections> SelectiveSearchModel::detectOmni(shared_ptr<Scene> scene)
		{
		...
		{
			cv::Mat kernel_filter_1(7,7, CV_32FC1,1);
			kernel_filter_1 = (cv::Mat_<double>(7,7) << 1.57130243e-04, 7.17839338e-04, 0, -1.76805171e-03, 0, 7.17839338e-04, 1.57130243e-04,
			...
			cv::Mat image_filtered_xx;
			cv::filter2D(regionMask, image_filtered_xx, -1, kernel_filter_1, cv::Point(-1,-1), 0, cv::BORDER_DEFAULT );
			cv::Mat xx_hist;
			float range[] = { hist_range_min, hist_range_max } ;
			const float * histRange = { range };
			bool uniform = true; 
			bool accumulate = false;
			cv::calcHist(&image_filtered_xx, 1, 0, cv::Mat(), xx_hist, 1, &histSize, &histRange, uniform, accumulate );
			return xx_hist;
		}

		cv::Mat get_hess_hist_xy(const cv::Mat & regionMask, int histSize, float hist_range_min, float hist_range_max)
		{
			cv::Mat kernel_filter_2(7,7, CV_32FC1,1);
			kernel_filter_2 = (cv::Mat_<double>(7,7) << 0.00017677, 0.00143568, 0.00321713, 0, -0.00321713, -0.00143568, -0.00017677,
			...
			cv::Mat image_filtered_xy;
			cv::filter2D(regionMask, image_filtered_xy, -1, kernel_filter_2, cv::Point(-1,-1), 0, cv::BORDER_DEFAULT );
			cv::Mat xy_hist;
			float range[] = { hist_range_min, hist_range_max } ;
			const float * histRange = { range };
			bool uniform = true; bool accumulate = false;
			cv::calcHist( &image_filtered_xy, 1, 0, cv::Mat(), xy_hist, 1, &histSize, &histRange, uniform, accumulate );
			return xy_hist;
		}
		...
		{
			cv::Mat kernel_filter_3(7,7, CV_32FC1,1);
			kernel_filter_3 = (cv::Mat_<double>(7,7) << 
			...
			cv::Mat image_filtered_yy;
			cv::filter2D(regionMask, image_filtered_yy, -1, kernel_filter_3, cv::Point(-1,-1), 0, cv::BORDER_DEFAULT );
			cv::Mat yy_hist;
			float range[] = { hist_range_min, hist_range_max } ;
			const float * histRange = { range };
			bool uniform = true; 
			...
			cv::calcHist(&image_filtered_yy, 1, 0, cv::Mat(), yy_hist, 1, &histSize, &histRange, uniform, accumulate );
			return yy_hist;
		}

		cv::Mat get_orientation_hist(const cv::Mat & regionMask, int histSize, float hist_range_min, float hist_range_max)
		{
			cv::Mat kernel_filter_1(3,3, CV_32FC1,1);
			kernel_filter_1 = (cv::Mat_<double>(3,3) << 1, 1, 1, 1, -8, 1, 1, 1, 1);
			cv::Mat kernel_filter_2(3,3, CV_32FC1,1);
			kernel_filter_2 = (cv::Mat_<double>(3,3) << 0,0,0,0,1,0,0,0,0);
			cv::Mat kernel_filter_3(3,3, CV_32FC1,1);
			kernel_filter_3 = (cv::Mat_<double>(3,3) << 1,2,1,0,0,0,-1,-2,-1);
			cv::Mat kernel_filter_4(3,3, CV_32FC1,1);
			kernel_filter_4 = (cv::Mat_<double>(3,3) << 1,0,-1,2,0,-2,1,0,-1);
			cv::Mat image_filtered_v1;
			cv::Mat image_filtered_v2;
			cv::Mat image_filtered_v3;	
			cv::Mat image_filtered_v4;
			int temp30;	
			//filtering
			cv::filter2D(regionMask, image_filtered_v1, -1, kernel_filter_1, cv::Point(-1,-1), 0, cv::BORDER_DEFAULT );
			cv::filter2D(regionMask, image_filtered_v2, -1, kernel_filter_2, cv::Point(-1,-1), 0, cv::BORDER_DEFAULT );
			cv::filter2D(regionMask, image_filtered_v3, -1, kernel_filter_3, cv::Point(-1,-1), 0, cv::BORDER_DEFAULT );
			cv::filter2D(regionMask, image_filtered_v4, -1, kernel_filter_4, cv::Point(-1,-1), 0, cv::BORDER_DEFAULT );
			...
			cv::Mat orientation_image = regionMask.clone();
		...
		*/
			...
			const float * histRange = { range };
			bool uniform = true; bool accumulate = false;
			cv::Mat orientation_image_hist;

			/// Compute the histograms:
			cv::calcHist( &orientation_image, 1, 0, cv::Mat(), orientation_image_hist, 1, &histSize, &histRange, uniform, accumulate );
		...
		cv::Mat get_diff_exci_hist(const cv::Mat & regionMask, int histSize, float hist_range_min, float hist_range_max)
		{
			...
			cv::Mat differential_excitation_image = regionMask.clone();
		...
		*/
			...
			bool uniform = true; 
			bool accumulate = false;
			cv::Mat differential_excitation_hist;

			/// Compute the histograms:
			cv::calcHist(&differential_excitation_image, 1, 0, cv::Mat(), differential_excitation_hist, 1, 
				         &histSize, &histRange, uniform, accumulate );
			return differential_excitation_hist;
		}

		void refineRegions(std::vector<std::vector<int> > sp, int total_masks, SelectiveSearchModel regions[], int min_height, int min_width)
		{
			...
			for(int i = 0; i < total_masks; i++)
			{
				if((regions[i].max_x - regions[i].min_x > min_width) && (regions[i].max_y - regions[i].min_y > min_height))
		...
		void createModel(std::vector<std::vector<int> > sp, int total_masks, const cv::Mat & grayMask, SelectiveSearchModel regions[], int histSize, 
			             float hist_range_min, float hist_range_max)
		{
			cv::Mat regionMask = grayMask.clone();
		...
		//		cout << i << " color" << endl;
				...
				bool uniform = true; 
				bool accumulate = false;
				cv::calcHist(&grayMask, 1, 0, cv::Mat(), regions[i].color_hist, 1, &histSize, &histRange, uniform, accumulate);
		...
		{
			if( 
				((a.min_x < b.min_x) && (b.min_x < a.max_x) && (a.min_y < b.min_y) && (b.min_y < a.max_y))
				||
		...
		std::vector<std::vector<int> > findNeighbors(SelectiveSearchModel regions[], int total_masks, const cv::Mat & regionMask, 
			                                         const std::vector<std::vector<int> > & sp)
		{
			...
			for(int i = 1; i < total_masks-1; i++)
			{
				for(int j = i+1; j < i+20; j++)
				{
					if(j < total_masks-2)
					{
						if(checkNeighbors(regions[i], regions[j]) && regions[i].validity && regions[j].validity)
						{
			/*				cout << i << " " << j << endl;
		...
		float calcSimilarities(const SelectiveSearchModel & a, const SelectiveSearchModel & b, float spSize)
		{
			...
			double bbsize = ((std::max(a.max_x, b.max_x) - std::min(a.min_x, b.min_x)) * (std::max(a.max_y, b.max_y) - std::min(a.min_y, b.min_y)));
			sim += 100 * ((bbsize - a.size - b.size) / spSize);
			return sim;
		}

		void mergeRegions(int value, SelectiveSearchModel regions[], const std::vector<std::vector<int> > & sp_neighbors, const cv::Mat &grayMask, 
		...
		*/
			...
			regions[sp_neighbors[value][0]].min_x = std::min(regions[sp_neighbors[value][0]].min_x, regions[sp_neighbors[value][1]].min_x);
			regions[sp_neighbors[value][0]].max_y = std::max(regions[sp_neighbors[value][0]].max_y, regions[sp_neighbors[value][1]].max_y);
			regions[sp_neighbors[value][0]].min_x = std::min(regions[sp_neighbors[value][0]].min_x, regions[sp_neighbors[value][1]].min_x);
			regions[sp_neighbors[value][0]].max_y = std::max(regions[sp_neighbors[value][0]].max_y, regions[sp_neighbors[value][1]].max_y);
			                                ...
			                                int minRegionSize, int histSize, float hist_range_min, float hist_range_max)
		{
			...
			int value = 10;
			std::vector<std::vector<int> > sp_neighbors;

			while(checkRounds(totals, regions, numRounds) && value > 0)
			{
				sp_neighbors = findNeighbors(regions, total_masks, img, sp);
		...
		//		cout << "value = " << value << endl;
			...
			std::cout << "Total bounding boxes = " << pts.size() << std::endl;
			return pts;		
		}
