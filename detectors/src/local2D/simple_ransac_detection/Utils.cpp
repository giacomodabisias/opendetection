...
namespace od {
  ...
  namespace l2d {
    ...
    void viewImage(cv::Mat image, std::vector<cv::KeyPoint> keypoints)
    {
    ...
    void drawObjectMesh1(cv::Mat image, const Mesh * mesh, const Model * model, PnPProblem * pnpProblem, cv::Scalar color)
    {
      ...
      cv::projectPoints(model->getPoints3d(), pnpProblem->getRVect(), pnpProblem->getTMatrix(), pnpProblem->getAMatrix(), pnpProblem->getDistCoef(), imgpoints);
    ...
    {
      std::vector<cv::Point2f> imgpoints;
      cv::projectPoints(model->getPoints3d(), R_vect, t_mat, A_mat, dist, imgpoints);
      draw2DPoints(image, imgpoints, color);
