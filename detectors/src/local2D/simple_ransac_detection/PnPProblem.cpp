...
PnPProblem::PnPProblem(double const params[], double const dists[])
{
  ...
  _dist = cv::Mat::zeros(4, 1, CV_64FC1);
  _dist.at<double>(0,0) = dists[0]; _dist.at<double>(1,0) = dists[1]; _dist.at<double>(2,0) = dists[2]; _dist.at<double>(3,0) = dists[3];
...
PnPProblem::PnPProblem(cv::Mat intrin, cv::Mat dist) {
  intrin.copyTo(_A_matrix);
  dist.copyTo(_dist);
                               ...
                               int flags)
{
  ...
  // Pose estimation
  bool correspondence = cv::solvePnP( list_points3d, list_points2d, _A_matrix, _dist, rvec, tvec,
                                     ...
                                     float reprojectionError, double confidence )    // Ransac parameters
{
  ...
  cv::solvePnPRansac( list_points3d, list_points2d, _A_matrix, _dist, rvec, tvec,
